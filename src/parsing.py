...
for id in id_list:
    ...
    # AtomArray - coord - tensor - file
    bb_coord = struc.coord(bb_atoms)
    ca_coord = struc.coord(ca_atoms)
    bb_tensor = torch.from_numpy(bb_coord)
    ca_tensor = torch.from_numpy(ca_coord)
    torch.save(bb_tensor, f"test_dataset/{id}_bb.pt")
    torch.save(ca_tensor, f"test_dataset/{id}_ca.pt")
